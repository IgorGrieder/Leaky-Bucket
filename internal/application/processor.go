...
func (p *ProcessorService) ProcessMutation(mutation domain.Mutation, ctx context.Context, user *domain.User) ([]domain.Mutation, error) {
	...
	if err != nil {
		slog.Error("error fetching pix keys check", slog.Any("error", err))
...
func (p *ProcessorService) FetchAndRefilTokens() error {
	...
	for {
		...
		if err != nil {
			slog.Error("failed to SCAN keys from Redis", slog.Any("error", err))
			return err
		}
		...
		for _, key := range keys {
			...
			if err != nil {
				slog.Error(fmt.Sprintf("Failed to refill token for key '%s': %v", key, err))
			}
		}
